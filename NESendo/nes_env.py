...
# setup the argument and return types for SetMasterVolume
_LIB.SetMasterVolume.argtypes = [ctypes.c_void_p, ctypes.c_float]
_LIB.SetMasterVolume.restype = None
# setup the argument and return types for SetAudioEnabled
_LIB.SetAudioEnabled.argtypes = [ctypes.c_void_p, ctypes.c_bool]
_LIB.SetAudioEnabled.restype = None
# setup the argument and return types for GetAudioBufferSize
_LIB.GetAudioBufferSize.argtypes = [ctypes.c_void_p]
_LIB.GetAudioBufferSize.restype = ctypes.c_int
# setup the argument and return types for GetAudioBuffer
_LIB.GetAudioBuffer.argtypes = [ctypes.c_void_p]
_LIB.GetAudioBuffer.restype = ctypes.POINTER(ctypes.c_float)
# setup the argument and return types for ClearAudioBuffer
_LIB.ClearAudioBuffer.argtypes = [ctypes.c_void_p]
_LIB.ClearAudioBuffer.restype = None
...
class NESEnv(gym.Env):
    ...
    def __init__(self, rom_path):
        ...
        # check that the mapper is implemented
            ...
            msg = 'ROM has an unsupported mapper number {}. please see https://github.com/Kautenja/NESendo/issues/28 for more information.'
    ...
    def step(self, action):
        ...
        truncated = False  # NES games don't have time limits, so truncated is always False
    ...
    def get_audio_buffer(self):
        """Get the current audio buffer from the emulator."""
        if not self._env:
            return None
        
        size = _LIB.GetAudioBufferSize(self._env)
        if size == 0:
            return None
        
        buffer_ptr = _LIB.GetAudioBuffer(self._env)
        if not buffer_ptr:
            return None
        
        # Convert C array to numpy array
        audio_data = np.ctypeslib.as_array(buffer_ptr, shape=(size,))
        return audio_data.copy()  # Return a copy to avoid memory issues
    
    def get_and_clear_audio_buffer(self):
        """Get and clear the audio buffer from the emulator."""
        if not self._env:
            return None
    ...
    def clear_audio_buffer(self):
        ...
        if self._env:
            _LIB.ClearAudioBuffer(self._env)
    ...
    def set_master_volume(self, volume):
        ...
        if self._env:
            _LIB.SetMasterVolume(self._env, ctypes.c_float(self.master_volume))
    ...
    def set_audio_enabled(self, enabled):
        ...
        if self._env:
            _LIB.SetAudioEnabled(self._env, ctypes.c_bool(enabled))
